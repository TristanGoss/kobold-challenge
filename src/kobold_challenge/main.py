...
def main():
    ...
    with tools.timer('classifying the rock types'):
        for rock_type in ['serpentinite', 'granodiorite', 'ultramafic']:
            bedrock_data = tools.classify_rock_type(gdf=bedrock_data, type=rock_type, new_col_name=f'is_{rock_type}')

    # Let's amalgamate the serpentinite and ultramafic results together, as advised in the instructions
    bedrock_data['is_serpentinite_or_ultramafic'] = bedrock_data['is_serpentinite'] & bedrock_data['is_ultramafic']
    ...
    tools.save_raster_to_disk(sep_map, transform, epsg, 'cobalt_rock_separation.tif')
